...
import {onMounted, ref} from "vue";
  ...
  let props = defineProps(["promptMessage","defName"])
  let text = ref();
...
<template>
  <div class="absolute left-0 top-0 w-screen h-screen bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-slate-900 rounded-lg px-3 py-3 text-center">
      ...
      <div class="flex-row">
        ...
        <a @click="$emit('input', text); text = '';" class="cursor-pointer mx-4 text-slate-900 bg-teal-500 px-1.5 py-0.5 rounded-lg hover:text-slate-900 hover:bg-teal-600">Submit</a>
      </div>
    </div>
  </div>
</template>
